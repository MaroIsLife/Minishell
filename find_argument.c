...
int	find_argument_three(char *s, t_source *src, int *i)
{
	if ((s[(*i)] == '>' || s[(*i)] == '<')
		&& src->aslash == 0 && src->dquotes == 0 && src->squotes == 0)
		return (find_file_name(&i, s, src));
	else if (s[(*i)] == ' ')
...
int	find_argument_two(char *s, t_source *src, int i)
{
	int	a;

	while (s[i] != '\n' && s[i] != '\0')
	{
		a = find_argument_three(s, src, &i);
		if (a == 1)
			continue ;
		else if (a == 2)
			break ;
